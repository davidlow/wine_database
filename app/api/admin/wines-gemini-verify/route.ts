...
import { verifyDuplicates } from '@/lib/wine-lookup/label-scan';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await verifyDuplicates(found.map(w => ({
      name: w!.name,
      producer: w!.producer,
    ...
    console.error('[POST /api/admin/wines-gemini-verify]', err);
